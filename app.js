...
function lessonText(y,u){if(y===1){let f=u?.focus||'';if(f==='huruf')return `Dengar bunyi, lihat huruf dan sebut perkataan mudah dalam konteks “${u.title}”. Belum perlu membina ayat.`;if(f==='suku')return `Dengar, sebut dan cantum suku kata menjadi perkataan mudah dalam konteks “${u.title}”.`;if(f==='kata')return `Kenal perkataan, padankan dengan makna atau gambar, kemudian gunakannya secara lisan.`;if(f==='frasa')return `Gabungkan dua atau tiga perkataan menjadi frasa yang bermakna.`;if(f==='ayat')return `Gunakan frasa yang sudah dikuasai untuk membina satu ayat mudah bermakna.`;if(f==='faham')return `Baca bahan sangat ringkas dan cari maklumat yang dinyatakan.`;if(f==='tulis')return `Tulis satu ayat mudah secara terkawal berdasarkan konteks.`;return `Belajar satu kemahiran asas secara berperingkat sebelum bergerak ke kemahiran seterusnya.`}return y===2?`Gunakan kosa kata unit untuk membina ayat lengkap dan tambah maklumat yang sesuai.`:y===3?`Cari idea utama, tambah idea sokongan dan susun 2–3 ayat yang berkaitan.`:y===4?`Kenal pasti isi, beri huraian dan contoh sebelum membina perenggan.`:y===5?`Rancang isi, huraian, contoh dan gunakan penanda wacana dengan tepat.`:`Rancang respons, bina aliran idea yang koheren, kemudian semak bahasa dan ketepatan.`}
...
(()=>{const oldLearn=learn;function cp(u,s){let m={huruf:['🔤','Dengar dan kenal bunyi','Sentuh perkataan. Sebut perlahan-lahan dan dengar bunyi awalnya.'],suku:['🧩','Cantum suku kata','Sebut setiap bahagian, kemudian cantum semula.'],kata:['🌳','Kenal perkataan','Baca atau dengar perkataan dan fahami maksudnya.'],frasa:['🧱','Bina frasa mudah','Gabungkan dua atau tiga perkataan menjadi frasa bermakna.'],ayat:['💬','Bina ayat mudah','Gunakan frasa yang sudah dikuasai untuk satu ayat mudah.']}[s[0]]||['📖','Belajar langkah demi langkah','Cuba satu kemahiran pada satu masa.'];return m}learn=function(){if(state.year!==1)return oldLearn();let u=un(),s=skills().find(x=>x[0]===u.focus)||sk(),c=cp(u,s);state.skill=Math.max(0,skills().indexOf(s));let ls=s[0]==='huruf'?['1 Dengar','2 Kenal','3 Sebut','4 Cuba']:s[0]==='suku'?['1 Dengar','2 Pecah','3 Cantum','4 Cuba']:['1 Faham','2 Belajar','3 Bina','4 Guna'];return shell(`<section class="page narrow v300-student"><div class="crumb">Tahun 1 › ${u.theme[0]} › Unit ${state.unit+1}</div><div class="lesson-head"><button class="back" data-view="curriculum">←</button><div><small>${u.theme[1]} ${u.theme[0]}</small><h1>${u.title}</h1><p>Fokus: ${s[2]} ${s[1]}</p></div></div><div class="lesson-layout v300-layout"><aside class="card v300-skillmap"><b>Laluan Saya</b>${skillList()}</aside><article class="card lesson v300-focus"><div class="stage-pills">${ls.map((x,i)=>`<span class="${i?'':'on'}">${x}</span>`).join('')}</div><div class="lesson-visual v300-visual">${c[0]}</div><small class="v300-kicker">AKTIVITI PERTAMA</small><h2>${c[1]}</h2><p class="lead">${u.prompt}</p><div class="v300-instruction">👩🏻‍🏫 ${c[2]}</div><div class="word-chips">${u.words.map(w=>`<button type="button" class="v300-word" data-say-word="${w}">${w}</button>`).join('')}</div><div id="v300Feedback" class="v300-feedback">💡 Tekan satu perkataan dan sebut bersama Cikgu Aira.</div><div class="v300-actions"><button class="pink" id="guided">Selesai Aktiviti →</button><button class="soft" data-view="practice">🎮 Cuba Aktiviti</button></div><div class="micro-status">${bar(s[1],score(key(s)))}</div><details class="v300-more"><summary>Untuk ibu bapa / guru</summary><p>${lessonText(1,u)}</p><p>Guided practice bukan mastery. Penguasaan memerlukan bukti independent dalam konteks baharu.</p></details></article></div></section>`)};const ob=bind;bind=function(){ob();$$('[data-say-word]').forEach(b=>b.onclick=()=>{let w=b.dataset.sayWord;b.classList.add('heard');let f=$('#v300Feedback');if(f)f.textContent=`🔊 ${w} — dengar bunyi awal dan sebut sekali lagi.`;try{if('speechSynthesis' in window){speechSynthesis.cancel();let x=new SpeechSynthesisUtterance(w);x.lang='ms-MY';x.rate=.72;speechSynthesis.speak(x)}}catch(e){}})};window.BAHASA_IPAD_V300={version:'300.0.0',project:'Bahasa AI',year1FoundationGuard:true,karanganAIDependency:false}})();

/* Bahasa AI ULTIMATE v350 — Year 1 interactive foundation lesson */
(()=>{
 const previousLearn350=learn;
 const foundationStages={huruf:['Dengar','Kenal','Sebut','Cuba'],suku:['Dengar','Pecah','Cantum','Cuba'],kata:['Lihat','Dengar','Padan','Cuba']};
 function speak350(t){try{if(!('speechSynthesis' in window))return false;speechSynthesis.cancel();const x=new SpeechSynthesisUtterance(t);x.lang='ms-MY';x.rate=.68;speechSynthesis.speak(x);return true}catch(e){return false}}
 function y1Interactive(u,s){
   const focus=s[0], words=(u.words||[]).slice(0,4), stages=foundationStages[focus];
   if(!stages)return null;
   const w=words[0]||u.title.split(' ')[0]||'saya', letter=(w[0]||'s').toUpperCase();
   const alternatives=[...new Set([letter,...['A','B','M','S','K'].filter(x=>x!==letter)])].slice(0,4);
   return `<section class="page narrow v300-student"><div class="crumb">Tahun 1 › ${u.theme[0]} › Unit ${state.unit+1}</div><div class="lesson-head"><button class="back" data-view="curriculum">←</button><div><small>${u.theme[1]} ${u.theme[0]}</small><h1>${u.title}</h1><p>Fokus: ${s[2]} ${s[1]}</p></div></div><div class="lesson-layout v300-layout"><aside class="card v350-skillmap"><b>Laluan Saya</b>${skillList()}</aside><article class="card lesson v300-focus"><div class="v350-stagebar">${stages.map((x,i)=>`<button class="v350-stage ${i?'':'on'}" data-v350-stage="${i}">${i+1} ${x}</button>`).join('')}</div><div id="v350Task" class="v350-task" data-stage="0"><div class="v350-letter">${letter}</div><small class="v300-kicker">AKTIVITI 1 · DENGAR</small><h2>Dengar bunyi dan perkataan</h2><p class="lead">Tekan perkataan. Dengar Cikgu Aira menyebutnya dengan perlahan.</p><div class="word-chips">${words.map(x=>`<button type="button" class="v300-word" data-v350-say="${x}">🔊 ${x}</button>`).join('')}</div><div id="v350Coach" class="v350-coach">👩🏻‍🏫 Dengar dahulu. Tidak perlu membina ayat.</div><button class="v350-primary" id="v350Next" disabled>Sudah dengar → Kenal huruf</button></div><details class="v300-more"><summary>Untuk ibu bapa / guru</summary><p>${lessonText(1,u)}</p><p>Aktiviti berpindah daripada input bunyi kepada pengecaman, sebutan dan percubaan kendiri. Guided practice bukan mastery.</p></details></article></div></section>`;
 }
 learn=function(){if(state.year!==1)return previousLearn350();const u=un(),s=skills().find(x=>x[0]===u.focus)||sk();state.skill=Math.max(0,skills().indexOf(s));return y1Interactive(u,s)||previousLearn350()};
 const priorBind350=bind;
 bind=function(){priorBind350();
   let heard=false,stage=0;const task=$('#v350Task');if(!task)return;
   const u=un(),s=skills().find(x=>x[0]===u.focus)||sk(),words=(u.words||[]).slice(0,4),w=words[0]||'saya',letter=(w[0]||'s').toUpperCase();
   function pills(){$$('[data-v350-stage]').forEach((b,i)=>{b.classList.toggle('on',i===stage);b.classList.toggle('done',i<stage)})}
   $$('[data-v350-say]').forEach(b=>b.onclick=()=>{heard=true;speak350(b.dataset.v350Say);b.classList.add('heard');$('#v350Coach').textContent=`🔊 “${b.dataset.v350Say}”. Dengar bunyi awalnya dan sebut perlahan-lahan.`;$('#v350Next').disabled=false});
   $('#v350Next').onclick=()=>{stage=1;pills();task.innerHTML=`<div class="v350-letter">${letter}</div><small class="v300-kicker">AKTIVITI 2 · KENAL</small><h2>Huruf manakah bunyi awal “${w}”?</h2><p class="lead">Pilih satu huruf.</p><div class="v350-choicegrid">${[...new Set([letter,'A','M','B'])].slice(0,4).map(x=>`<button class="v350-choice" data-letter="${x}">${x}</button>`).join('')}</div><div id="v350Coach" class="v350-coach">💡 Dengar “${w}” sekali lagi jika perlu.</div><button class="v350-listen" id="v350Replay">🔊 Dengar semula</button>`;bindStage1()};
   function bindStage1(){ $('#v350Replay').onclick=()=>speak350(w);$$('[data-letter]').forEach(b=>b.onclick=()=>{if(b.dataset.letter===letter){b.classList.add('good');$('#v350Coach').textContent=`✅ Betul. “${w}” bermula dengan huruf ${letter}.`;setTimeout(()=>stage2(),450)}else{b.classList.add('bad');$('#v350Coach').textContent='Cuba lagi. Dengar bunyi awal perkataan.'}})}
   function stage2(){stage=2;pills();task.innerHTML=`<div class="v350-letter">${letter}</div><small class="v300-kicker">AKTIVITI 3 · SEBUT</small><h2>Sebut bersama Cikgu Aira</h2><p class="lead">Dengar, kemudian sebut “${w}” dengan suara kamu sendiri.</p><button class="v350-listen" id="v350Model">🔊 Cikgu Aira sebut</button><div class="v350-coach">👩🏻‍🏫 Selepas kamu menyebutnya, tekan butang di bawah. Tiada markah untuk suara kamu pada tahap ini.</div><button class="v350-primary" id="v350Spoken">Saya sudah sebut →</button>`;$('#v350Model').onclick=()=>speak350(w);$('#v350Spoken').onclick=stage3}
   function stage3(){stage=3;pills();const opts=[w,...words.filter(x=>x!==w)].slice(0,4);task.innerHTML=`<small class="v300-kicker">AKTIVITI 4 · CUBA SENDIRI</small><h2>Cari perkataan yang bermula dengan ${letter}</h2><p class="lead">Pilih tanpa bantuan.</p><div class="v350-choicegrid">${opts.map(x=>`<button class="v350-choice" data-word="${x}">${x}</button>`).join('')}</div><div id="v350Coach" class="v350-coach">🌱 Ini percubaan kendiri ringkas. Belum dikira mastery penuh.</div>`;$$('[data-word]').forEach(b=>b.onclick=()=>{if((b.dataset.word[0]||'').toUpperCase()===letter){b.classList.add('good');$('#v350Coach').textContent='🌟 Bagus! Kamu sudah lengkapkan aktiviti asas ini.';task.classList.add('v350-success');try{addEvidence('guided')}catch(e){};save()}else{b.classList.add('bad');$('#v350Coach').textContent='Belum tepat. Cuba lihat huruf pertama setiap perkataan.'}})}
 };
 window.BAHASA_IPAD_V350={version:'350.0.0',project:'Bahasa AI',interactiveFoundation:true,speechModel:true,speechRecognitionRequired:false,karanganAIDependency:false};
})();
